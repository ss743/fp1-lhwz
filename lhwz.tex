...
In order to determine the half life of a radioactive elements, usually the number of decays per second (decay rate) is meassured and the change over time of the decay rate observed. The half-life can then be determined from this characteristic exponential behaviour. However, this method isn't useful to determine the half-life of samples with a very long half-life, e.g. ${}^{238}$Uranium with a half-life of $T_{1/2}$=4,468$\cdot 10^9$years,  as hardly any change will be observed. Instead the activitiy of the sample is meassured, that presents the quantity desrcibing the derivative of the law of decay given by the following relation: A=$\dot{N}$=ln2N/$T_{1/2}$ where N is the known number of radioactive nuclides.


In this experiment this method is used to determine the half-life of the $\alpha$-emitter ${}^{147}$Samarium and the $\beta$-emitter ${}^{40}$Kalium both having very long half-lives. Additionally two differnet correction procedures are applied to consider back-scattering and self-absorption effects. 
In the case of Samarium, we take advantage of the constant range of the  $\alpha$-radiation and the known surface area while we make use of the mass dependancy for Kalium.
...
The mean life-time of an Atom describing the average length of time that an element remains in the set, is directly dependant on the exponential decay constant and defined the following way:
\begin{align}
\tau=\frac{1}{\lambda}
\end{align}
...
\end{align}
Another characteristic Quantitiy for radioactive decays is the Activity $A$ which describes the Number of Decays per second meassured in Becquerel ( $Bq=1/s$):
\begin{align}
A = \lambda N = \frac{N}{\tau} = \frac{N\ln2}{T_{1/2}}
\end{align}

\subsection{Radioactive Decay}
Nuclei of unstable Atoms (so-called parent radionuclide) transform into another Nucleus (daughter nculide) under transmission of ionized radiation, depending on the kind of decay taking place. The most important and relevant types of decay for this experiment are presented in the following sections:

\subsubsection{$\alpha$-decay \label{alpha}}

A nuclei that decays via an $\alpha$-decay will admit an $\alpha$-particle, a helium nucleus consisting of two protons and two neutrons. Nulcei that dcay this way are usually heavy nuclei, their mass number is reduced by four while there charge number is reduced by two:
\begin{align}
...
The $\beta$-decay is descirbed by different processes of the weak interaction
...
A $\beta^-$-decay implies a neutron transfomring into a proton in the nucleus under transmission of an electron and an electron-antineutrino:
...
Charged particles can ionize or excite Atoms and Molecules. The maximum energy that can be transferred is dependant of its velocity and mass.
...
As $\alpha$-particles have a very limited range in matter, the effect of self-absorption cab not be neglected. Only particles whose distance travelled to leave the sample r is shorter than the range R in the sample. 
...
With the thickness d=$\frac{V}{F}$, the surface area F, the volume V, the activity A and the activity per volume A=$\frac{A}{V}$.

\subsubsection{Correction for $\beta$-radiation}
...
The decay rate n, that origins from a volume element $F\cdot dx$ in a distance x is dependant on the surface area F, the specific activity $A_s$, the density $\rho$ and the absorption coefficient  $\mu$:  
\[dn\sim A_s\cdot F\cdot \rho\cdot e^{-\mu x}dx\]

Neglecting the energy-dependany of $\mu$ and integrating over height  h=$\frac{m}{\rho F}$ leads to:
...
solving for a and pluggin that in we obtain:
...
\vskip 20 pt
\gra[0.8]{Versuchsaufbau}{Versuchsaufbau diesen Versuchs}
\subsection{Used samples}

\subsubsection{Characteristic of the Counter tube}
To record the characteristid of the Counter Tube ${}^{238}$Uranium is used. It makes up 99.275\% of the natural Uranium.
...
As $\beta$-radiator we use ${}^{40}$Kalium with a relativ  mass portion of 0.0118\%. The sample used in the Experiment is KCl. 
