\documentclass[12pt,liststotoc]{scrartcl}
\usepackage{fancyhdr}
\usepackage{geometry}
\usepackage{ucs}
...
In order to determine the half life of a radioactive elements, usually the number of decays per second (decay rate) is measured and the change over time of the decay rate observed. The half-life can then be determined from this characteristic exponential behaviour. However, this method isn't useful to determine the half-life of samples with a very long half-life, e.g. ${}^{238}$Uranium with a half-life of $T_{\frac12}$=4,468$\cdot 10^9$ years, as hardly any change will be observed. Instead the activity of the sample is measured with a counter tube, that presents the quantity describing the derivative of the law of decay given by the following relation: A=$\dot{N}$=ln2N/$T_{\frac12}$ where N is the known number of radioactive nuclides.
...
\tau=\frac{1}{\lambda}
\end{align}
which again is directly related to the half-live $T_{\frac12}$ given by:
\begin{align}
T_{\frac12} = \ln2\cdot \tau = \frac{\ln2}{\lambda}
\end{align}
...
\begin{align}
A = \lambda N = \frac{N}{\tau} = \frac{N\ln2}{T_{\frac12}}\label{activity}
\end{align}
...
\subsubsection{Electron  Capture}\label{ec}
...
The range of $beta$-radiation is much bigger, that is to say a few meters in air and a few millimetres in matter. \textsuperscript{\cite{staat}}
 ...
 \gra[0.85]{Zaehlrohr}{Functioning of the Counting Tube \cite{staat} \label{Zählrohr}}
...
\begin{minipage}{\textwidth}
	...
	\captionof{figure}{For a maximum range R of $\alpha$-particles in the sample there exists a maximum angle $\theta_{max}$ at which particles can still leave the sample. \textsuperscript{\cite{staat}}}
\end{minipage}
\vskip0.5cm
...
\end{align}
with the Avogadro constant $N_A$, the natural abundance $h_{rel}$ of $^{147}$Sm and the diameter $d$ of the sample (see section \ref{auswertungSamarium}).
...
the actual half-life will be given by (see section \ref{auswertungSamarium}):
\begin{align}
	T_{\frac12} &= \frac{T_{\frac12,\beta^-}}{1.12}\\
...
In this experiment we use a "Methan-Durchflusszählrohr" as described in chapter \ref{CT} to detect the $\gamma$-rays from the samples.
The pulses detected by the counter tube are proportional to the primary ionization. They are then converted into voltage signals by a preamplifier (Amplifier 142PC, Firma Ortec) that are again proportional to the primary ionization. This signal is then converted into a square wave signal with an Amplifier (Amplifier 571, Ortec) using differentiation and integration with a specific time constant (shaping time) $\mu$. This signal is then transferred to a single channel analyzer (TSCA 551, Ortec) with a setting for lower and upper level that determines in which energy range an event is registered. The counter (BNC 2120, National Instrument) is connected with  the program LabView on the computer which allows to vary all essential settings i.e. Voltage, counting time and intervals.\textsuperscript{\cite{anleitung}}
\vskip 20 pt
\gra[0.8]{Versuchsaufbau}{Experimental Set-Up \textsuperscript{\cite{staat}}}
\subsection{Used samples}
 In the following the different samples and their characteristic decays used in this experiment are explained.
...
With a half-life of $T_{\frac12}({}^{238}U)=4.468\cdot 10^9$a it transforms via  $\alpha$-decay to ${}^{234}$Thorium. The energy of the $\alpha$-particle is about 4MeV in this process. According to \ref{alpha} this process is described as:

\[{}^{238}_{92}U \rightarrow {}^{234}_{90}Th + {}^4_2He\]

Thorium then decays via $\beta$-decay  to ${}^{234}$Palladium with a half-time of  $T_{\frac12}({}^{234}Th)$=24,1d, which again decays via  $\beta$-decay into ${}^{234}$Uranium. According to chapter \ref{beta} this processes are written as follows:
...
In this experiment ${}^{147}$Samarium that makes up 15\% of the natural Samarium is used as $\alpha$-radiator. The used sample consists of $Sm_2O_3$ and has a purity of 99\%. Samarium has a half-life of  $T_{\frac12}({}^{147}Sm)=1.06\cdot10^{11}$a. It decays into ${}^{143}$Neodym (stable) transmitting an $\alpha$-particle with an energy of about 2MeV which is written as (see section \ref{alpha}):
...
${}^{40}$Kalium can decay via two decays: Either via $\beta^-$-decay (see section \ref{beta}) to ${}^{40}$Calcium or via electron capture (see section \ref{ec}) to ${}^{40}$Argon:
...
In the following the different measurements that were necessary to determine the half-lives are described.

\subsubsection{Counter Tube Characteristics with ${}^{238}$Uranium}\label{durchfuehrunguran}
\begin{itemize}
	\item First of all the variable settings of the electronic have to be chosen in a way so that a useful signal is generated. In order to achieve such a signal, the signal is observed with an oscilloscope after passing through the preamplifier, the main amplifier and the single channel analyzer (SCA) separately. To optimize the separation of our signal and noise the lower level of the SCA has to be high enough to eliminate pules caused by noise and at the same time low enough to still register small signal pulses. The chosen settings need to be kept constant during the entire experiment.
	
		\textbf{shaping amplifier}: coarse gain: 100 \hspace{1.5cm} gain: 1.1 \hspace{1cm} shaping time: 6$\mu$s\\
		\textbf{timing SCA}: \hspace{1.13cm} lower level: 3.6 \hspace{0.95cm} window: 0.06 \hspace{2.2cm} delay: 4.09
	
	\item The characteristic of the counter tube is recorded with ${}^{238}$Uran (see chapter \ref{uranium}). The settings chosen for the measurement program are the following: initial voltage $U_{anf}$=1000V, final voltage $U_{end}$=4000V, voltage intervals $U_{step}$=100V and counting time per measurement/voltage t=50s
	
	\item We then measure the background by measuring with an empty Aluminium dish with the same settings as before except the counting time per voltage being t=100s
\end{itemize}
...
\begin{itemize}
	...
	\item We now measure the $\alpha$-Plateau of ${}^{147}$Sm with an initial voltage of $U_{anf}$=1900V and a final voltage of $U_{end}$=3400V (a bit larger than the beginning of the $\beta$-Plateau), voltage intervals $U_{step}$=100V and counting time per measurement/voltage t=200s. With this measurement we want to determine an ideal working point to measure the activity of ${}^{147}$Sm.
	
	\item At a working point of 2500V we measure the activity of ${}^{147}$Sm.
      As we want to obtain a statistical error no larger than 2\% we calculate the measuring time needed from the expected value for the counting rate n (obtained from the Plateau measurement):$n_{2500}$=0.48$s^{-1}$=5400s (see \ref{SaCa}) 
...
\begin{itemize}
	...
	\item At a working point of U=3500V  the activity is measured for different masses. The measuring time is determined as before, by aiming at having a statistical error no larger than 2\%, we obtain t=600s (see \ref{KaCa}). As the uncertainty for higher activities will be higher due to the Poisson error, we measure more points for higher masses. We then planned to also cover values until 0 gram, but didn't manage to do so as we ran out of time. The mass is measures with a precision scale.
...
In the following sections we will analyze the data we recorded in our measurements to determine the working point and to determine the half-lives of the samples. The Analysis is done with the programming language R, the code can be found in the attachement (see \ref{Sourcecode}).

\subsection{Counter Tube Characteristics}

\subsubsection{Uranium}
As discussed in chapter \ref{durchfuehrunguran} we use $^{238}$Uranium to get the characteristics of the counter tube.

\gra{zaehlrohr-uran}{Counter tube characteristics of $^{238}$U\label{curan}}
In figure \ref{curan}, that is obtained by plotting the data from \ref{LM}, we see the expected behaviour, namely two plateuas for the different decays. The first plateau comes from the $\alpha$-decay of $^{238}$U and the second plateau shows the $\beta$-decay of $^{234}$Th and $^{234}$Pd (see section \ref{uranium}). As it can be seen in figure \ref{curan} the plateaus are between $U=1900\,\mathrm{V}$ and $U=2600\,\mathrm{V}$ for $\alpha$ and between $U=3200\,\mathrm{V}$ and $U=4000\,\mathrm{V}$ for $\beta$.
The error on the counting rate is here and in the following chapters given by the Poisson error on the counts that is calculated in the following manner: 
\begin{align}
s_n=n\frac{s_N}{N}=\frac Nt\frac1{\sqrt N}=\frac{\sqrt{N}}{t}=\sqrt{\frac nt} \label{Poisson} 
\end{align}

\newpage
\subsubsection{Background}\label{untergrund}
The background radiation is measured by putting an empty aluminium dish inside the counter tube (see chapter \ref{durchfuehrunguran}).

\gra{zaehlrohr-untergrund}{Counter tube characteristics of the background radiation\label{background}}
In figure \ref{background}, that is obtained from \ref{geh halt}, we see that the background is very small. The background counting rates are about 3 orders of magnitude smaller than the counting rates of the uranium sample. So we would not be able to see any change in the position of the plateaus if we substracted the background from the counter tube characteristics of the uranium sample.\\

Additionally the time for measuring the background is very short, so we don't get significant background counting rates. As a consequence, we see in figure \ref{background} that the counting rates scatter a lot and probably won't better our values.
\newpage
\subsubsection{Samarium}
As a next step the working point is determined (see chapter \ref{durchfuehrungsamarium}). Therefore we measure the counter tube characteristics of Samarium in a span starting at the beginning of the $\alpha$-plateau of uranium and ending on the beginning of the $\beta$-plateau of uranium.
...
In figure \ref{csamarium} we plotted the counter tube characteristics of $^{147}$Sm using the data presented in \ref{SaCa}, once original and once corrected by the background seen in figure \ref{background}. Since the corrected data scatters a lot and has very big error bars (as discussed in chapter \ref{untergrund}) we can't really use the corrected data to set our working point. So we assumed an almost linear behavior in the range of $U=2300\,\mathrm{V}$ and $U=2900\,\mathrm{V}$. Considering the $\alpha$-plateau from figure \ref{curan} we set the working point for Samarium to $U=2500\,\mathrm{V}$
\newpage
\subsubsection{Kalium}
We also measured the counter tube characteristics of Kalium to determine the working point for the Kalium measurements (see chapter \ref{kaliumdurchfuehrung}). We started at the beginning of the $\beta$-plateau and stopped at $U=4000\,\mathrm{V}$.

\gra{zaehlrohr-kalium}{Counter tube characteristics of $^{40}$K \textcolor{blue}{(corrected with the background)}\label{ckalium}}

In figure \ref{ckalium} that is obtained with the data in \ref{KaCa}we see that the counting rate seems to be constant between $U=3300\,\mathrm{V}$ and $U=4000\,\mathrm{V}$ with one outlier at $U=3800\,\mathrm{V}$. The correction does not essentially change this behavior, so we choose our working point at $U=3500\,\mathrm{V}$.
\newpage
\subsection{Half-life of Samarium}\label{auswertungSamarium}\label{HS}
...
\begin{align*}
	N_{\text{A}}&=6.022\cdot10^{23}\,\mathrm{mol^{-1}}\,^{\cite{dem}}\\
	h_{\text{rel}}&=0.1487\,^{\cite{anleitung}}\\
	R\rho&=0.04025\,\mathrm{\frac{kg}{m^2}}\\
	m_{\text{mol}}&=0.34872\,\mathrm{\frac{kg}{mol}}\,^{\cite{chemie1}}
\end{align*}
where $R\rho$ is calculated with the known values for air\textsuperscript{\cite{anleitung}}: $$R\rho=R_{\text{Air}}\rho_{\text{Air}}\cdot\frac{\sqrt{m_A}}{\sqrt{m_{A,\text{Air}}}}=R_{\text{Air}}\rho_{\text{Air}}\cdot\frac{\sum{p_i}\cdot\sqrt{m_A}}{\sum{p_{i,\text{Air}}}\cdot\sqrt{m_{A,\text{Air}}}}$$
We use the following measurement data:
\begin{align*}
	d&=(2.877\pm0.002)\cdot10^{-2}\mathrm{m}\\
	n&=(0.416\pm0.009)\,\mathrm{s^{-1}}
\end{align*}
where the error on n is calculated according to \ref{Poisson} and the error on $d$ is given by gaussian error propagation out of the error for the single measurements $d_i$: 
\begin{align*}
 s_d=\frac1{\sqrt{n}}s_{d_i}
\end{align*}
...
\end{align*}
The error on $T_{\frac12,\text{Sm}}$ is obtained with the gaussian error propagation on the Poisson errors of the counting rate: 
\begin{align*}
...
\end{align*}
\newpage
\subsection{Half-life of Kalium}\label{auswertungKalium}
...
\end{align}
We hence plot the obtained data, (see \ref{bb})the counting rate over the mass, as presented in figure \ref{kalium}.

\gra{kalium}{Counting rate in dependence of the mass with an exponential fit\label{kalium}}

Fitting the expected curve (see \ref{kalium.R} for Sourcecode) as given in formula (\ref{fitfunction}) to the data (see figure \ref{kalium}) then gives us the following parameters:
\begin{align*}
	a&=4.4\,\mathrm{s^{-1}}\\
	b&=2119\,\mathrm{kg^{-1}}
\end{align*}
with the following covariance matrix that is used to calculate the error on the half-life:
\begin{align*}
\sigma&=\left(
	\begin{matrix}
		0.25&-705\\
...
\begin{align*}
	...
	f_B&=1.29\,^{\cite{anleitung}}\\
	m_{\text{mol}}&=0.07455\,\mathrm{\frac{kg}{mol}}\,^{\cite{chemie2}}
\end{align*}

So we can calculate the half-life with formula (\ref{halftimeKa}) (see chapter \ref{betacorrection}):
\begin{align*}
	T_{\frac12,\text{Ka}}&=\frac{\ln2N_Ah_{rel}f_B}{2\cdot1.12abm_{mol}}\\
	&=1.3\cdot10^{9}\,\text{years}
\end{align*}
The error on $T_{\frac12,\text{Ka}}$ is again obtained with the gaussian error propagation: 
\begin{align*}
...
\end{align*}
where $\sigma_{ij}$ are the components of the covariance matrix.\\
...
\subsection{Sourcecode \label{Sourcecode}}
...
\subsection{Experimental data \label{bb}}
\subsubsection{Uranium Characteristics \label{LM}}
\lstinputlisting{data/uran2.txt}
\subsubsection{Background Characteristics \label{geh halt}}
\lstinputlisting{data/Kalium1.txt}
\subsubsection{Samarium Characteristics \label{SaCa}}
\lstinputlisting{data/Samarium1.txt}
\subsubsection{Kalium Characteristics \label{KaCa}}
\lstinputlisting{data/Kalium1.txt}
...
\bibitem{chemie1}
\emph{http://de.webqc.org/molecular-weight-of-sm2O3.html}, 12.10.2016

\bibitem{chemie2}
\emph{http://de.webqc.org/mmcalc.php}, 12.10.2016
